...
class ExerciseList extends Component{
    constructor(props){
        ...
        componentDidMount(){
            axios.get('http://localhost:3001/exercises/').then(res=>{this.setState({exercises:res.data})}).catch(err=> console.log(err));
        }

        deleteExercise = id =>{
            axios.delete('http://localhost:3001/exercises/'+id).then(res=>{console.log(res.data)}).catch(err=> console.log(err));
            this.setState({exercises: this.state.exercises.filter(e=>e._id !== id)});
        };
